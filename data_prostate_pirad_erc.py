...
def prepare_data(input_folder,
    ...
    for folder in os.listdir(input_folder):
        ...
        if os.path.isdir(folder_path) and 't2_tse_tra.nii.gz' in os.listdir(folder_path):
            if 'segmentation_' + labeller + '.nii.gz' in os.listdir(
                    folder_path) or 'segmentation_tra_' + labeller + '.nii.gz' in os.listdir(folder_path):
    ...
    for idx in range(idx_start, idx_end):
        ...
        # print('image shape before: {}'.format(image_normalized.shape))
        ...
        # ==================
        label[label > 2] = 0
        # print('before: ', np.unique(label), label.shape)

        # ======================================================
        ### PROCESSING LOOP FOR SLICE-BY-SLICE 2D DATA ###################
        # ======================================================
        # print(image_hdr.get_zooms())
        ...
        for zz in range(image.shape[2]):
            ...
            label2d_rescaled = rescale(np.squeeze(label[:, :, zz]),
                                       ...
                                       mode='constant',
                                       anti_aliasing=False)
            print('orignal: ', np.unique(label[:, :, zz]))
            print('rescaled: ', np.unique(label2d_rescaled))
            if zz == 0:
                nx_after_list.append(image2d_rescaled.shape[0])
                ny_after_list.append(image2d_rescaled.shape[1])
            ...
            # ============
            # mean = np.mean(image2d_rescaled_rotated)
            ...
            # print('cropped: ', np.unique(label2d_rescaled_rotated_cropped))
    ...
    hdf5_file.create_dataset('nx_after', data=np.asarray(nx_after_list, dtype=np.uint16))
    hdf5_file.create_dataset('ny_after', data=np.asarray(ny_after_list, dtype=np.uint16))
...
def crop_or_pad_slice_to_size(slice, nx, ny):
    ...
    if x > nx and y > ny:
        ...
        if x <= nx and y > ny:
            slice_cropped = slice[:, y_s:y_s + ny]
            slice_cropped = np.pad(slice_cropped, ((x_c, x_c), (0, 0)), mode='symmetric')
        elif x > nx and y <= ny:
            slice_cropped = slice[x_s:x_s + nx, :]
            slice_cropped = np.pad(slice_cropped, ((0, 0), (y_c, y_c)), mode='symmetric')
        else:
            slice_cropped = np.pad(slice, ((x_c, x_c), (y_c, y_c)), mode='symmetric')
...
if __name__ == '__main__':
    data_root = '/usr/bmicnas01/data-biwi-01/bmicdatasets-originals/Originals/USZ/Prostate/'
    input_folder = '../../../../../' + data_root
    preprocessing_folder = 'test_data/'
    ...
    data_usz = load_data(input_folder=input_folder,
                         preproc_folder=preprocessing_folder,
                         idx_start=idx_start,
                         idx_end=idx_end,
                         size=image_size,
