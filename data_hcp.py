...
def prepare_data(input_folder,
    ...
    print("len filename: {}".format(len(filenames)))
    ...
    for idx in range(idx_start, idx_end):
        ...
        image = np.swapaxes(image, 1,
                            2)  # swap axes 1 and 2 -> this allows appending along axis 2, as in other datasets

        # ==================
        # read the label file
        # ==================
        label, _, _ = utils.load_nii(label_path)
        label = np.swapaxes(label, 1,
                            2)  # swap axes 1 and 2 -> this allows appending along axis 2, as in other datasets
        ...
        for zz in range(image.shape[2]):
            ...
            print(image_normalized[:,:,zz].shape, image2d_rescaled.shape)
...
if __name__ == '__main__':
    data_root = '/usr/bmicnas01/data-biwi-01/bmicdatasets-originals/Originals/HCP/3T_Structurals_Preprocessed/'
